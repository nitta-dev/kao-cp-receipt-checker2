...
- confidence: 全体の読み取り信頼度（0.0〜1.0）。正直に自己評価すること。高く盛らない。
  - 1.0: 全項目を100%確信を持って読み取れた。推測ゼロ。
  - 0.9: ほぼ全て読み取れた。省略表記の解釈は含むがOK
  - 0.8: 概ね読み取れたが、一部不鮮明な箇所がある
  - 0.7: 画像品質に問題あり、または文字が切れて読めない箇所が複数ある
  - 0.6以下: 大部分が読み取り困難、または重要項目（店舗名・日付・金額）が不明瞭
  - ★省略表記（ビTB等）を正しく読めた場合、それだけでは0.9未満にしない
  - ★本当に読めない・判別不能な箇所がある場合のみ0.9未満にする

## 信頼度の判定基準（加点方式: 0.5からスタート）
- 店舗名が明確に読める → +0.1
- 日付が明確に読める → +0.1
- 全商品名が読める（省略表記でもOK、下記参照） → +0.2
- 金額が全て明確に読める → +0.1
- 上記すべて満たす → ボーナス +0.05（= 1.05 → 1.0にキャップ）
- ★省略表記は減点しない: POS/レシートの省略表記（ビTB、ビTH、THS等）は業界標準の略記であり推測ではない。省略表記が正しく読み取れていれば減点不要
- 画像が不鮮明・傾き・影あり → -0.05〜-0.15
- is_cp_targetの判定に推測が入った → -0.1（ただし上記の既知省略パターンは推測に含めない）
- 金額の読み取りに推測が入った → -0.1
- 文字が切れていて完全に読めない場合のみ → -0.1〜-0.2
