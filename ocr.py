...
- confidence: 全体の読み取り信頼度（0.0〜1.0）。正直に自己評価すること。高く盛らない。
  ...
  - 0.9: ほぼ全て読み取れたが、1〜2箇所だけ軽微な推測あり（例: 省略表記の解釈）
  - 0.8: 概ね読み取れたが、商品名の省略や一部不鮮明な箇所がある
  - 0.7: 複数の商品名が省略・切り詰めで推測が必要、または画像品質に問題あり
  - 0.6以下: 大部分が読み取り困難、または重要項目（店舗名・日付・金額）が不明瞭
  - ★少しでも迷った・推測した箇所があれば、0.9未満にすること
...
- 商品名に省略・切り詰めがある → -0.05〜-0.15（省略の程度による）
- 画像が不鮮明・傾き・影あり → -0.05〜-0.15
- is_cp_targetの判定に推測が入った → -0.1
- 金額の読み取りに推測が入った → -0.1
